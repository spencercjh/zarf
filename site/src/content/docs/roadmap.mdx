...
## The Zarf project roadmap for upcoming work toward v1.0.0
...
- **New import structures**: Component imports will move to `ZarfComponentConfig`, and skeleton packages will be replaced by remote components.
...
- **Remote values file sources** ([#4223](https://github.com/zarf-dev/zarf/issues/4223)): Enabling values files to be loaded from HTTP/HTTPS URLs or OCI registries, in addition to local filesystem paths.
...
- **Expose `ZARF_TEMP` in actions** ([#4219](https://github.com/zarf-dev/zarf/issues/4219)): Making the Zarf temporary directory accessible in action contexts via the new templating system (e.g. `{{ .Zarf.tmpDir }}`), eliminating the need for brittle filesystem-search workarounds in airgap deployments.
...
- `zarf tools wait-for resource` now waits for readiness by default, rather than resource existence ([#4077](https://github.com/zarf-dev/zarf/issues/4077)). This behavior will also be reflected in the v1beta1 schema.
...
- **Git fallback** ([#2772](https://github.com/zarf-dev/zarf/issues/2772)): Improving resilience when Git operations encounter errors during package deployment.
...
Targeted security improvements that complement the broader signing and schema work:
